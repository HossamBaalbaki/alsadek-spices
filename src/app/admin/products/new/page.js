...
export default function NewProductPage() {
  ...
  const [form, setForm] = useState({
    ...
    nameEn: "", // bundle only
    nameAr: "", // bundle only
    descriptionEn: "", // bundle only
    descriptionAr: "", // bundle only
    price: "", // bundle only
    originalPrice: "", // bundle only
    images: [""], // bundle images or override
    featured: false,
    bestSeller: false,
    active: true,
    labels: {
  ...
  const handleVariantChange = (i, field, value) => {
    setVariants((prev) => {
      ...
      if (field === "weightLabel") {
        next[i].grams = parseWeightToGrams(value);
      }
      return next;
    });
  };
    ...
    try {
      ...
      if (form.type === "single") {
        ...
        if (!cleanVariants.length) {
          setError("Add at least one valid variant (e.g. 100g / 250g).");
          setLoading(false);
          return;
        }
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          href="/admin/products"
          className="px-4 py-2 rounded-xl border border-stone-200 text-stone-600 hover:bg-stone-50 font-semibold text-sm"
        >
          ← Back to Products
        </Link>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        <div className="bg-white rounded-2xl border border-stone-200 p-6 flex flex-col gap-4">
          <h3 className="font-black text-stone-800 text-lg border-b border-stone-100 pb-3">
            Core Product Setup
          </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-xs text-stone-400 mt-1">
                  Name/description are auto-copied from selected stock.
                </p>
              </div>
              <div className="bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm">
                ...
                <p className="text-stone-600">
                  Sell / g: <b>{Number(selectedStock?.sellPricePerGram || 0).toFixed(3)}</b>
                </p>
                <p className="text-stone-600">
                  Cost / g: <b>{Number(selectedStock?.costPerGram || 0).toFixed(4)}</b>
                </p>
              </div>
            </div>
          )}
            ...
            {variantPreview.map((v, i) => (
              <div key={i} className="grid grid-cols-1 md:grid-cols-4 gap-3 items-center">
                ...
                <div className="text-sm text-stone-700">
                  Price preview: <b>{Number(v.price || 0).toFixed(2)} QAR</b>
                </div>
                {variants.length > 1 ? (
            ...
            {bundleItems.map((b, i) => (
              <div key={i} className="grid grid-cols-1 md:grid-cols-4 gap-3 items-center">
                ...
                {bundleItems.length > 1 ? (
                  <button type="button" onClick={() => removeBundleItem(i)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg">
                    ✕
                  </button>
                ) : (
                  <div />
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 flex flex-col gap-4">
          <div className="flex items-center justify-between border-b border-stone-100 pb-3">
            ...
            <button type="button" onClick={addImage} className="px-3 py-1.5 bg-amber-700 text-white rounded-lg text-xs font-bold">
              + Add Image
            </button>
          </div>
          {form.images.map((img, i) => (
            <div key={i} className="flex items-center gap-3">
              <input
                ...
                className="flex-1 px-4 py-2.5 rounded-xl border border-stone-200 text-sm"
              />
