...
function ProductCard({ product, rank, priority = false }) {
      ...
      <div className={`product-card h-full flex flex-col ${soldOut ? "sold-out" : ""}`}>
        ...
        <div className="p-3 flex flex-col flex-1 gap-2">
          ...
          <div className="flex items-center gap-1">
            <span className="text-xs tracking-tight" aria-label={`${product?.rating || 0} out of 5 stars`}>
              {Array.from({ length: 5 }, (_, i) => (
                <span key={i} className={i < Math.round(product?.rating || 0) ? "text-amber-400" : "text-stone-200"}>★</span>
              ))}
            </span>
            <span className="text-xs text-stone-400">({product?.reviewCount || 0})</span>
          </div>
